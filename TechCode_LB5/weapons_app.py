...
def load_weapons() -> List[Weapon]:
    if not os.path.exists(DATA_FILE):
        ...
        demo = [
            Weapon("Rifle", "AK-47", 25000, 30, 600),
            Weapon("Pistol", "PM", 12000, 8, 120),
            Weapon("Knife", "Bayonet", 1500, 0, 0),
        ]
        save_weapons(demo)
        return demo
    try:
        with open(DATA_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
        return [Weapon.from_dict(x) for x in data]
    except Exception:
        QMessageBox.warning(None, "Load error", "Не удалось прочитать weapons.json. Будет создан новый файл.")
        save_weapons([])
        return []


def save_weapons(items: List[Weapon]) -> None:
...
class AddEditDialog(QDialog):
    def __init__(self, parent=None, weapon: Weapon | None = None):
        ...
        if weapon:
            self.cmb_class.setCurrentText(weapon.wclass)
            self.le_name.setText(weapon.name)
            self.sb_price.setValue(weapon.price)
            self.sb_capacity.setValue(weapon.capacity)
            self.sb_rapidity.setValue(weapon.rapidity)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Weapons")
        self.resize(520, 380)

        self.items: List[Weapon] = load_weapons()
    ...
    def _on_add(self):
        ...
        if dlg.exec_() == QDialog.Accepted:
            self.items.append(dlg.get_weapon())
            save_weapons(self.items)
            self.refresh_ui()
